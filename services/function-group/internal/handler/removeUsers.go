...
type RemoveUsersBody = models.UserIdWrapper[[]string]

func RemoveUsers(c *fiber.Ctx) error {
	c.Accepts("application/json")
	functionGroupId := c.Params("id")

	body := new(RemoveUsersBody)
	...
	if !isAdmin {
		log.Debug().Str("body", string(c.Body())).Msg("Only admins can remove user")
		return fiber.NewError(fiber.StatusUnauthorized, "Only admins can remove user")
	}

	functionGroup, err := client.RemoveUsers(body.Data, functionGroupId, c.UserContext())
	if err != nil {
		log.Debug().Err(err).Str("body", string(c.Body())).Msg("User remove failed")
		return fiber.NewError(fiber.StatusInternalServerError, "User remove failed")
	}
